...
`RequestOptions.extensions`, стабильный `operationId` и промежуточный обработчик событий.
...
низкоуровневому `itd.request()` с `encrypt` требуется явный `operationId`; запрос `raw`, даже на
совпадающий URL, не считается встроенным ресурсом.
...
Имена встроенных шифров собраны в `CipherName` — замороженном объекте, как enum

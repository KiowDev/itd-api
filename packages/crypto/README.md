...
- **REST-уведомления.** Нормализация списка выполняется после HTTP-плагинов, поэтому
  `notification.preview` из `itd.notifications.list()` не расшифровывается. Получите сам
  пост или комментарий либо используйте событийный middleware, как показано выше.

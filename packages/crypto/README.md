...
Поддерживается `itd-api >=0.8.0 <1.0.0`: crypto использует namespace
`RequestOptions.extensions`, стабильный `operationId` и событийный middleware.
...
Тот же экземпляр подключается к нормализованным событиям отдельно от HTTP-конвейера:
...
itd.notifications.events.on('notification', ({ notification }) => {
  console.log(notification.secret?.text);
});
```

Промежуточный обработчик обходит `context.update`, поэтому работает с любым предметным
каналом, в нормализованных данных которого есть поддерживаемые текстовые поля.
HTTP-плагин тем же способом обрабатывает `preview` уведомлений из
`itd.notifications.list()`.
...
Подключаемый feature может разрешить шифрование собственных полей через
`annotations.crypto.requestFields`. Например, чат объявляет `['message']`; функция нормализации
ответа при этом crypto-плагину не передаётся.

...
комментарии и ответы, реакции, подписки, уведомления, магазин, удаление и восстановление.
Для магазина `seed` принимает `shopProducts` и `shopOrders`; доступны каталог, доставка,
создание и оплата заказов, а также доступ по коду из письма.
...
`mock.requests` скрывает токены, cookie, пароли, OTP и токены капчи.

...
- **Разметка.** `spans` вы задаёте по видимому тексту. Нагрузка крепится в конец, поэтому
  смещения обложки не сдвигаются, — но у поста, зашифрованного без обложки, размечать
  нечего.

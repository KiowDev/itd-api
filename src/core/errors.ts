...
export class ItdApiError extends ItdError {
  ...
  /**
   * Временная ошибка сервера, которую политика повторов считает повторяемой: `429` или `5xx`.
   *
   * Повторит ли библиотека саму операцию, зависит ещё от её `retrySafety`: `5xx` на
   * небезопасной операции не повторяется, `429` — повторяется всегда.
   */

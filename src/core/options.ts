...
export interface RuntimeOptions {
  ...
  /**
   * Таймаут одной транспортной попытки в мс. По умолчанию 30000 — столько же использует
   * сайт итд.com. `0` снимает ограничение.
   *
   * Охватывает сборку заголовков, хуки `onRequest` и `onResponse`, перехватчики попытки,
   * запрос и чтение тела ответа. Ожидание очереди, паузы между повторами и хук `onError`
   * в него не входят. Истечение даёт {@link ItdTimeoutError} с `budget: 'attempt'`;
   * safe- и idempotent-операции повторяются.
   */
  timeout?: number | undefined;
  /**
   * Общий срок логической операции в мс: очередь, повторы, обновление авторизации, плагины
   * и хуки. По умолчанию не ограничен.
   *
   * Истечение даёт {@link ItdTimeoutError} с `budget: 'deadline'` и не повторяется. Тот же
   * эффект даёт `signal: AbortSignal.timeout(ms)`, но с {@link ItdAbortError}.
   */
...
export interface RequestOptions {
  ...
  /** Таймаут одной транспортной попытки только для этого запроса, мс. */
  timeout?: number | undefined;
  /** Общий срок только для этого запроса, мс. См. {@link RuntimeOptions.deadline}. */
  deadline?: number | undefined;
  /** Дополнительные заголовки. */
  headers?: Record<string, string> | undefined;
  /**
   * Повторы только для этого запроса. Переопределяют глобальную настройку `retry`.
   * Лестницу пауз после `429` не затрагивают — как и глобальное `retry: false`.
   */
  retry?: RetryOptions | false | undefined;
  /**

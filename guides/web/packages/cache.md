...
Запросы с собственным `signal`, `timeout` или `deadline` используют готовые ответы, но не
объединяются между собой: отмена или срок одного запроса не прерывает остальные.
...
всеми его разделами. `attachNotificationEvents()` затрагивает аккаунт создавшего канал клиента.
...
Токен и заголовки в ключ не входят. Также не учитываются `signal`, `timeout`, `deadline`
и настройки повторов. Запрос с несериализуемым JSON-телом выполняется без кэширования.

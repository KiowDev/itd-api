...
Полный каталог операций, подключение к нескольким клиентам и привязка к realtime описаны в
...
уже проходит через его обёртку, — но не дольше `shutdownTimeout`. По истечении срока teardown
выполняется всё равно, а `unuse()` и `dispose()` отклоняются `ItdStateError` с именем плагина.

...
interface OperationTransformContext {
  // Общий signal пользовательской отмены, deadline операции и dispose() клиента.
  signal: AbortSignal;
}
...
первый срабатывает также по `deadline` операции и при `dispose()` клиента, второй остаётся
сигналом, переданным вызывающим кодом. `timeout` ограничивает отдельную транспортную
попытку и на обёртку операции не действует.

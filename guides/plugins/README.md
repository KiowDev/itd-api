...
Событийный middleware расшифровывает те же поля в нормализованных обновлениях. Доступны
...
`operationId` и HTTP-метод изменять нельзя: они входят в контракт операции. Для другого
семантического действия feature или resource должен запустить отдельную операцию.
...
`next()` разрешено вызвать один раз: повторный вызов завершает операцию ошибкой с именем
плагина, иначе один вызов `posts.create()` мог бы создать два поста.
...
`operations.get()` возвращает только публичные метаданные операции: HTTP-метод, retry policy,
bucket и plugin annotations. Контракт преобразования ответа намеренно скрыт: плагину достаточно
готового результата `next()` и собственных метаданных.
...
| --- | ---: | --- | --- | --- |
| `OperationTransformer` | один раз на операцию | публичный нормализованный | да, готовым результатом | обычно нет |
...
уже проходит через его обёртку, — но не дольше `shutdownTimeout`. После этого teardown всё
равно запускается, однако ожидание его собственного результата тоже ограничено оставшимся
общим сроком. `unuse()` и `dispose()` отклоняются `ItdStateError` с именем плагина и
незавершённой стадией.
...
`dispose()` делает окончательную очистку, вызывает teardown всех плагинов изнутри наружу
и переводит клиент в терминальное состояние. После него новые запросы и `use()` завершаются
с `ItdStateError`. Отправка уже накопленной телеметрии выполняется до teardown, поэтому
operation transformer может увидеть `telemetry.dwell` и `telemetry.interaction` после
начала `dispose()`. `await using` вызывает `dispose()` автоматически.

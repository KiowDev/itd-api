...
Если исходный текст уже размечен, `parseMarkdown()` и `parseHtml()` превращают его в
обычный текст и `spans`:

```ts
import { parseHtml, parseMarkdown, post } from 'itd-api';

const fromMarkdown = parseMarkdown('**важно:** [подробности](https://example.com)');
const fromHtml = parseHtml('<strong>важно:</strong> <a href="https://example.com">подробности</a>');

await itd.posts.create(fromMarkdown);
await itd.posts.create(post().html('<strong>или сразу через билдер</strong>'));
```

У `PostBuilder` есть короткие формы `.markdown(source)` и `.html(source)`. Они заменяют
текущий текст и разметку так же, как `.markup()`.

Поддерживаемый Markdown:

- жирный, курсив, зачёркивание и спойлер;
- inline- и fenced-код;
- ссылки и HTTP(S)-автоссылки;
- цитаты;
- `<u>` для подчёркивания.
...
- безопасные ссылки `a`;
- переводы строк и блочные элементы;
- `data-spoiler` и классы с окончанием `spoiler`.

Это намеренно небольшой, предсказуемый набор, а не полноценный браузерный Markdown/HTML
движок. Неподдерживаемый Markdown остаётся текстом. Неизвестные HTML-теги игнорируются с
сохранением безопасного текста, а содержимое `script`, `style`, `iframe`, `object` и SVG
удаляется. Ссылками по умолчанию становятся только адреса с протоколами `http:` и `https:`;
дополнительные протоколы можно разрешить через `allowedLinkProtocols`.

Парсеры не исполняют HTML, не сохраняют атрибуты событий и всегда проверяют полученные
диапазоны. Смещения остаются корректными для кириллицы и emoji, потому что считаются в
UTF-16.
...
Поэтому библиотека читает разметку комментариев, но не отправляет ручные spans. Результат
`parseMarkdown()` или `parseHtml()` предназначен для постов и других API, принимающих
`spans`, но не для создания комментариев.

...
Модуль можно передать контейнеру один раз — он будет установлен и на вручную добавленные,
и на восстановленные аккаунты:
...
await accounts.restore();
accounts.addAccount('новый', { auth: token });
```

`create()` вызывается отдельно для каждого клиента: состояние API и фоновые ресурсы между
аккаунтами не разделяются. Сначала устанавливаются общие плагины, затем модули в порядке
массива `features`, и только после успеха клиент появляется в контейнере. Если фабрика или
`setup()` бросает ошибку, клиент освобождается и аккаунт не публикуется.

Бакеты модуля в общей очереди едины для всех аккаунтов: их ограничения запоминаются при
первом запросе, и модуль, объявивший тот же бакет с другими `limit`, `concurrency` или
`rps`, получит `ItdConfigError` при первом же запросе.

Динамического добавления или удаления модулей у уже созданных аккаунтов нет. Базовый
`ItdAccounts` не пытается вывести расширенный тип клиента из произвольного массива;
типизированный доступ предоставляет фабрика конкретного продуктового модуля.

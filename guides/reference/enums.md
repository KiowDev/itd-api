...
Категория товара в каталоге магазина.
...
| `Preorder` = `'preorder'` | доступен по предзаказу |
| `SoldOut` = `'soldout'` | распродан |

## ShopConsentKind

Вид согласия покупателя.

| Значение | Описание |
|---|---|
| `Offer` = `'offer'` | принятие условий оферты |
| `PersonalData` = `'personal_data'` | обработка персональных данных |
| `Cookie` = `'cookie'` | использование cookie |
...
| Значение | Описание |
|---|---|
| `New` = `'new'` | создан, ожидает оплаты или обработки |
| `Paid` = `'paid'` | оплачен |
| `Shipping` = `'ship'` | передан в доставку |
| `Done` = `'done'` | выполнен |
| `Cancelled` = `'cancelled'` | отменён |
| `Refunded` = `'refunded'` | средства возвращены |
| `Expired` = `'expired'` | срок оплаты истёк |

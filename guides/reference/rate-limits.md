...
Очередь заводится на каждую пару «хост — бакет». Задача занимает слот бакета, затем общий
слот хоста, поэтому пауза одного бакета не задерживает остальные, а суммарное число
...
| `bucketOverrides` | `{ 'files.upload': { concurrency: 1 } }` | поправки к лимиту и одновременности отдельных бакетов; неизвестное имя — `ItdConfigError` |
...
считает запросы, занявшие слот бакета, включая те, что ещё ждут общего слота хоста
и в сеть не ушли; `pending` — ждущие слота самого бакета.

Снимок переживает `close()` — серверный счётчик от закрытия клиента не сбрасывается,
и после повторной работы темп продолжается с накопленного состояния. `dispose()` его
очищает.

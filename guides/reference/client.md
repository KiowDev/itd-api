...
```
Подключает плагин — обёртку вокруг запроса и нормализованного результата публичного метода
...
`pacing` выбирает реакцию на остаток. `react` не задерживает ничего, пока в бакете есть
запас, и придерживает исчерпанный бакет на `60000 / limit` миллисекунд. `smooth` держит
...
Таблица лимитов — в [«Ограничениях частоты»](/reference/rate-limits).

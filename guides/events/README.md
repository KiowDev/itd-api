...
событие никто не подписан, ошибка записывается через настроенный `logger` или в консоль.
...
идентификатор пользователя есть хотя бы у одного элемента `notification.actors`. Дополнительную проверку

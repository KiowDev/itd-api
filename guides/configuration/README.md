...
ранее созданного потока завершаются с `ItdStateError`. Повторный `dispose()` безопасно
возвращает тот же результат очистки. Для автоматического освобождения ресурсов доступен
`await using`.

...
в минуту. Единый планировщик хоста выбирает готовый бакет и одновременно проверяет
...
Хосты разделяются по итоговому `URL.origin`, а не по имени сервиса: alias одного origin
делят лимитер, другой origin и разовый `baseUrl` получают свои. `buckets: false` оставляет
одну очередь на хост; в этом режиме `bucketConcurrency` и все поля `bucketOverrides` не действуют,
...
`dispose()` дополнительно отменяет незавершённые запросы, отключает плагины и вызывает их
teardown, после чего клиент терминален. Новые запросы, `use()`, `defineService()`,
повторный `connect()` ранее созданного канала событий завершается с `ItdStateError`.
Записи, накопленные до вызова, всё же отправляются: их финализация выполняется через ещё
установленные плагины, до отмены запросов, и не даёт пользовательским вызовам обойти запрет.
...
Оба метода ждут чужой код — обработчики потока и операции, вошедшие в обёртки плагинов, —
не дольше `shutdownTimeout` (по умолчанию 10 секунд). Зависший обработчик остановить нельзя,
поэтому по истечении срока ожидание прекращается: ресурсы освобождаются, а метод отклоняется
`ItdStateError`, называющим плагин или транспорт потока, который удерживал остановку.

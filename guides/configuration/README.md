...
в минуту. Очередь заводится на каждую пару «хост — бакет»: пауза одного бакета не
задерживает остальные, а суммарная одновременность остаётся равной `concurrency`.

```
запрос
  ├─ уровень бакета: bucketConcurrency, своя пауза
  └─ уровень хоста:  concurrency, rps  →  транспорт
```
...
одну очередь на хост; в этом режиме `bucketConcurrency` и `bucketOverrides` не действуют,

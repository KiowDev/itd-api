...
Маршруты итд.com разбиты на группы, у каждой свой лимит запросов в минуту. Очередь
заводится на каждую пару «хост — группа»: исчерпание одной группы не задерживает
остальные, а суммарная одновременность остаётся равной `concurrency`.

```
запрос
  ├─ уровень группы: bucketConcurrency, своя пауза
...
- `React` — по умолчанию. Задержек нет, пока в группе есть остаток; при `remaining: 0`
  группа ждёт `60000 / limit` миллисекунд.
- `Smooth` — ровный темп в пределах лимита группы: `429` не возникает, но задержки идут
  с первого запроса.
- `Off` — остаток на темп не влияет, остаётся только пауза после `429`.

После `429` работает лестница пауз `retryDelays` — по умолчанию 1, 5, 30, 60 и 90 секунд.

Таблица групп, поправки `bucketOverrides`, своё правило выбора группы и снимок

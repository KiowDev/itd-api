...
`close()` закрывает realtime-потоки и отменяет запросы, которые ещё ждут очереди.
Уже начавшиеся запросы завершаются. Клиент после этого можно использовать снова.
